...
struct s_browser_data *s_browser_data_new(const char *address,
  ...
  const char *type)
{
  struct s_browser_data *data = x_malloc(sizeof(struct s_browser_data));
...
static void _s_browser_resolver_cbk(AvahiServiceResolver *resolver,
  x_unused AvahiIfIndex interface, x_unused AvahiProtocol protocol,
  AvahiResolverEvent event, const char *name, const char *type,
  const char *domain, x_unused const char *host_name,
  const AvahiAddress *address, uint16_t port, AvahiStringList *txt,
  x_unused AvahiLookupResultFlags flags, struct s_browser *browser)
{
  x_return_if_fail(browser);
...
struct s_browser *s_browser_new(struct s_client *client,
  ...
  void *userdata)
{
  x_return_val_if_fail(client, NULL);
  x_return_val_if_fail(data, NULL);
  x_return_val_if_fail(funcs, NULL);

  struct s_browser *browser = x_malloc(sizeof(struct s_browser));
...
error:
  x_log_error("failed to allocate a browser instance\n");
