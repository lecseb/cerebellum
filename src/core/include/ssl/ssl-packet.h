...
#ifndef _INCLUDE_SSL_SSL_PACKET_H_
# define _INCLUDE_SSL_SSL_PACKET_H_
...
static inline struct s_ssl_packet *s_ssl_packet_new(const uint8_t *payload,
  uint32_t size)
{
  x_return_val_if_fail(payload, NULL);

  struct s_ssl_packet *packet = x_malloc(sizeof(struct s_ssl_packet));
  packet->payload = x_malloc(sizeof(uint8_t) * size);
...
#endif /* !_INCLUDE_SSL_SSL_PACKET_H_ */
