...
#ifndef _INCLUDE_SSL_SSL_H_
# define _INCLUDE_SSL_SSL_H_
...
{
  x_return_val_if_fail(funcs, -EINVAL);
  x_return_val_if_fail(funcs->connection, -EBADE);
  x_return_val_if_fail(funcs->error, -EBADE);
  x_return_val_if_fail(funcs->read, -EBADE);
  return 0;
}
...
{
  x_return_val_if_fail(certificate, NULL);
  x_return_val_if_fail(private_key, NULL);

  SSL_load_error_strings();
  SSL_library_init();
  x_return_val_if_fail(RAND_poll(), NULL);

  SSL_CTX *context = SSL_CTX_new(SSLv23_server_method());
  x_return_val_if_fail(context, NULL);
  ...
  if (!SSL_CTX_use_certificate_chain_file(context, certificate) ||
    ...
    x_log_error("failed to initialize the ssl layer\n");
