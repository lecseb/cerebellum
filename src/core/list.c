...
{
  x_return_val_if_fail(list, 0);
  x_return_val_if_fail(llist, 0);
...
static struct s_list *_s_list_new(struct s_list *prev, void *data,
  struct s_list *next)
{
  struct s_list *list = x_malloc(sizeof(struct s_list));
  list->data = data;
  list->previous = prev;
  list->next = next;
...
{
  x_return_val_if_fail(func, list);
...
{
  x_return_val_if_fail(data, list);
...
{
  x_return_val_if_fail(data, list);
  x_return_val_if_fail(_s_list_check(list, sibling), list);
...
{
  x_return_val_if_fail(data, list);
  x_return_val_if_fail(list, list);
...
{
  x_return_val_if_fail(list, list);
  x_return_val_if_fail(llist, list);
  x_return_val_if_fail(_s_list_check(list, llist), list);
...
{
  x_return_val_if_fail(list && list->previous == NULL, list);
...
struct s_list *s_list_concat(struct s_list *list1,
  struct s_list *list2)
{
  x_return_val_if_fail(list1 && list1->previous == NULL, list1);
  x_return_val_if_fail(list2 && list2->previous == NULL, list1);
...
{
  x_return_val_if_fail(list && list->previous == NULL, NULL);
...
{
  x_return_val_if_fail(data, NULL);
